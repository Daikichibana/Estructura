...
public:
	...
	{	Naturales c;
		...
		void leer(DataGridView^ grilla, TextBox^ fil,TextBox^ col){
			...
			i=0;
			Naturales x;
			while(j<System::Convert::ToInt32(fil->Text)){
			while(j <System::Convert::ToInt32(fil->Text)){
				x.Set_entero(System::Convert::ToInt32(grilla->Rows[i]->Cells[i]->Value));
				matriz[i][j] = x;
				j++;
			}
			i++;
		...
		bool Verificar(Matriz y){
			if((y.tamano_fila==tamano_fila)&&(y.tamano_columna==tamano_columna))
			{
				return true;
			} else{
				return false;
